...
const app = createApp({
    data() {
        return {
            loadingInicial: true, erroConexao: false, erroCritico: '',
    ...
    methods: {
        ...
        resetarCache() { localStorage.clear(); window.location.reload(); }, // CURA TELA BRANCA
        ...
        carregarDb() { if(db) { 
            db.ref('system/users').on('value', s => { this.usuarios = s.val() ? Object.values(s.val()) : []; this.verificarSessao(); }, e => { this.erroConexao = true; }); 
            ...
            db.ref('system/config').on('value', s => { this.config = s.val() || { destinos: [], rota: ['Geral'] }; if(!this.config.cores) this.config.cores = { hortifruti: