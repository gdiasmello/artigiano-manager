...
const app = createApp({
    ...
    computed: {
        ...
        pedidosAgrupados() {
            ...
            this.itensParaPedir.forEach(p => {
                ...
                // LOGICA DE EXIBIÇÃO DA LINHA
                let textoItem = "";
                if (p.somenteNome) {
                    textoItem = `- ${p.nome}`; // Sem quantidade se marcado somenteNome
                } else {
                    textoItem = `- ${calc.qtd} ${p.unC} ${p.nome}`;
                }
                
                if (p.descricao) textoItem += ` (${p.descricao})`; // Adiciona descrição se houver

                if(this.analisarHistorico(p)) textoItem += " (⚠️ Cobrar: Chegou?)"; // NOVA MENSAGEM
    ...
    methods: {
        ...
        enviarZap(destId, itens, isSegunda) { 
            ...
            const dataF = agora.toLocaleDateString();
            const horaF = agora.toLocaleTimeString().slice(0,5);
            
            const h = { id: this.gerarId(), data: dataF, hora: horaF, usuario: this.usuarioLogado.nome, destino: nomeDestino, itens: (isSegunda ? "[2ª] " : "") + itens.map(i=>i.texto.replace('- ','')).join(', ') }; 
        ...
        aplicarPermissoesPadrao() { 
            // GERENTE AGORA PODE EDITAR ITENS
        ...
        prepararEdicaoProduto(p) {
            ...
            // Garante que o array de locais existe
        ...
        adicionarProduto() { 
            ...
            this.salvarProdutoUnitario(p); 
            alert("Salvo!"); 
            this.novoProd.nome = ''; this.novoProd.descricao = ''; this.novoProd.locaisSelecionados = []; 
        },
        ...
        excluirProduto(id) { if(confirm("Excluir item permanentemente?")) { db.ref('store/products/' + id).remove(); this.cancelarEdicaoProduto(); } },
        ...
        tentarRemoverLocal(idx) {
            ...
            // Verifica se tem produtos neste local
            // (Para simplificar na V69, remove direto se confirmado)
            if(confirm("Remover local?")) { this.config.rota.splice(idx,1); this.salvarConfig(); }
        },
        carregarDb() { if(db) { 
            ...
            db.ref('system/feriados').on('value', s => { this.feriados = s.val() ? Object.values(s.val()) : []; if(this.feriados.length===0) { const l=[{id:'1',data:'2026-12-10',nome:'Aniv. Londrina'},{id:'2',data:'2026-06-15',nome:'Padroeiro'},{id:'3',data:'2026-12-25',nome:'Natal'},{id:'4',data:'2026-01-01'