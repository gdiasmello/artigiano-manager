...
var app = Vue.createApp({
    ...
    methods: {
        ...
        fazerLogin: function() {
            ...
            if(u) { self.sessaoAtiva = true; } else { alert("Utilizador ou senha incorretos!"); }
        },
        ...
        enviarZap: function(destino) {
            ...
            this.itensFiltrados.forEach(function(item) {
                var estoqueTotal = 0;
                for (var l in item.contagem) { estoqueTotal += parseFloat(item.contagem[l] || 0); }
                
                if (estoqueTotal < item.meta) {
                    var falta = (item.meta - estoqueTotal).toFixed(1);
            ...
            if (!encontrouAlgo) {
                alert("O stock está completo. Nada para pedir!");
                return;
            }
